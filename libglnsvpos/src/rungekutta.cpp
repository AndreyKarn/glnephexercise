...
int RK(uint32_t N, double h, struct Y_s* Y) {
    ...
    for (uint32_t k = 1; k < N; k++)
    {
        ...
        Y2.F1 = Y->F1 + h * k1->F1 / two;
        Y2.F2 = Y->F2 + h * k1->F2 / two;
        Y2.F3 = Y->F3 + h * k1->F3 / two;
        Y2.F4 = Y->F4 + h * k1->F4 / two;
        Y2.F5 = Y->F5 + h * k1->F5 / two;
        Y2.F6 = Y->F6 + h * k1->F6 / two;

        k2 = diffs (0 + h / 2, Y2);

        Y3.F1 = Y->F1 + h * k2->F1 / two;
        Y3.F2 = Y->F2 + h * k2->F2 / two;
        Y3.F3 = Y->F3 + h * k2->F3 / two;
        Y3.F4 = Y->F4 + h * k2->F4 / two;
        Y3.F5 = Y->F5 + h * k2->F5 / two;
        Y3.F6 = Y->F6 + h * k2->F6 / two;
        ...
        knextstep->F1 = h_6 * ( k1->F1 + two * k2->F1 + two * k3->F1 + k4->F1 );
        knextstep->F2 = h_6 * ( k1->F2 + two * k2->F2 + two * k3->F2 + k4->F2 );
        knextstep->F3 = h_6 * ( k1->F3 + two * k2->F3 + two * k3->F3 + k4->F3 );
        knextstep->F4 = h_6 * ( k1->F4 + two * k2->F4 + two * k3->F4 + k4->F4 );
        knextstep->F5 = h_6 * ( k1->F5 + two * k2->F5 + two * k3->F5 + k4->F5 );
        knextstep->F6 = h_6 * ( k1->F6 + two * k2->F6 + two * k3->F6 + k4->F6 );
